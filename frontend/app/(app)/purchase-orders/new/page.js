...
export default function NewPurchaseOrderPage() {
  ...
  useEffect(() => {
    let cancelled = false
    api.get('/purchase-orders/next-number', { date: orderDate }).then((res) => {
  ...
  return (
    <div className="flex h-full flex-col">
      <ControlPanel
        breadcrumb="Purchase"
        title={nextNumber ? `New Purchase Order — ${nextNumber}` : 'New Purchase Order'}
        ...
        <FormSheet className="max-w-[1100px]">
          <FormSection>
            <FormGrid>
              ...
              <FormField label="Vendor" required className="sm:col-span-2">
