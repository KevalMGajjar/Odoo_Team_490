...
class OfflineStorage {
  ...
  static Future<void> init() async {
    ...
    // String-typed box for small app settings (e.g. the saved server URL).
  ...
  SyncMeta? getSyncMeta([String? userId]) {
    ...
    if (userId != null) {
      ...
      if (raw != null) return SyncMeta.fromJson(Map<String, dynamic>.from(_retype(raw) as Map));
    }
    if (metaBox.isNotEmpty) {
      final latest = metaBox.values.last;
      return SyncMeta.fromJson(Map<String, dynamic>.from(_retype(latest) as Map));
    }
    return null;
  }
  ...
  /// Recursively retype anything Hive hands back.
  ///
  /// Hive deserialises maps as `Map<dynamic, dynamic>`. A top-level
  /// `Map<String, dynamic>.from(...)` only fixes the outer layer, so nested
  /// structures — invoice lines, journal items, the embedded customer — stayed
  /// dynamic and every `as Map<String, dynamic>` cast inside the models threw.
  ///
  /// It only showed up after a restart: immediately following a sync the boxes
  /// still hold the original decoded JSON objects, so the casts pass. Read the
  /// same data back from disk and the app died on launch with a white screen.
  static dynamic _retype(dynamic value) {
    if (value is Map) {
      return value.map((k, v) => MapEntry(k.toString(), _retype(v)));
    }
    if (value is List) {
      return value.map(_retype).toList();
    }
    return value;
  }
  ...
  List<Map<String, dynamic>> getAll(String boxName) {
    ...
    return box.values
        .map((m) => Map<String, dynamic>.from(_retype(m) as Map))
        .toList();
  }
