...
class AppUser {
  ...
  factory AppUser.fromJson(Map<String, dynamic> json) => AppUser(
        ...
        // Email is informational only — sign-in is by Login ID. Kept nullable
        // so a user without one can never crash session restore.
  ...
  /// Roles must match the backend exactly (admin | accountant | user).
  /// These previously read 'invoicing_user' and 'contact', names the backend
  /// dropped — so both getters were silently always false, and a portal user
  /// was shown the full internal staff UI.
  bool get isAdmin => role == 'admin';
  bool get isAccountant => role == 'accountant';
  bool get isPortal => role == 'user';

  /// Internal staff see company-wide data; a portal user only ever sees their own.
  bool get isInternal => isAdmin || isAccountant;
}
